...
Title:
Migrate Target -> Targets in Actions\FatigueAction.cs
...
Files:
CardBattleEngine\Actions\FatigueAction.cs

Requirements:
Apply these substitution rules to every occurrence of `.Target` (singular) in this file. Specifically, the known site `Target = context.SourcePlayer` in an object initializer becomes `Targets = [context.SourcePlayer]`.

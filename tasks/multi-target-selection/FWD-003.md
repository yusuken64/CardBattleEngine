...
Title:
Migrate Target -> Targets in Actions\EquipWeaponAction.cs (mechanical rename only)
...
Files:
CardBattleEngine\Actions\EquipWeaponAction.cs

Requirements:
Apply these substitution rules to every occurrence of `.Target` (singular) in this file. Specifically, a line like `targets = [context.Target];` becomes `targets = context.Targets ?? [];`, preserving the existing fan-out-per-single-target pattern exactly.

Task ID:
FWD-007

Model:
haiku

Title:
Migrate Target -> Targets in Actions\TriggerEffectAction.cs (mechanical rename only)

Purpose:
Part of the solution-wide mechanical rename from `ActionContext.Target` (singular) to `ActionContext.Targets` (list). No Phase B functional work is planned for this file.

Dependencies:
CTX-000

Consumes:
CTX-000: `ActionContext.Targets` field shape (`List<IGameEntity>`)

Produces:
(none — leaf mechanical fixup)

Files:
CardBattleEngine\Actions\TriggerEffectAction.cs

Requirements:
Apply these substitution rules to every occurrence of `.Target` (singular) in this file. Specifically, `Target = context.Target` becomes `Targets = context.Targets` (a reference/list copy, not a re-wrap into a new single-element list).

| Old code pattern | New code pattern |
|---|---|
| `x.Target != null` | `x.Targets is { Count: > 0 }` |
| `x.Target` (read as a value) | `x.Targets?.FirstOrDefault()` |
| `Target = context.Target` (copying the whole context's target through) | `Targets = context.Targets` |
| `Target = <expr>` in an object initializer, `<expr>` provably non-null and not already a list-typed source | `Targets = [<expr>]` |
| `Target = null` | `Targets = null` |

Do NOT use `.Single()` anywhere. Do not change any logic beyond this substitution — this is a pure rename, no behavior change.

Constraints:
- Scope this diff to exactly this one file.
- Pure rename only.

Acceptance Criteria:
- No bare `.Target` (singular) reference remains anywhere in this file.
- Every substitution follows the table above with correct null/list semantics, correctly distinguishing "copy the whole list through" from "wrap a single non-null entity in a new list."

Validation:
This file alone cannot make the full solution build (many sibling files still reference the old field until their own tasks land) — that's expected. Confirm via search that no bare singular `.Target` reference remains in this file. Full solution build and test-suite pass is validated by the PHASEA-EXIT task once all sibling tasks land.

Handoff:
No downstream task specifically depends on this file beyond the overall PHASEA-EXIT integration checkpoint.

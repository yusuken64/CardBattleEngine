...
Title:
Migrate Target -> Targets in CardBattleEngine.Test\ValidActionTest.cs
...
Files:
CardBattleEngine.Test\ValidActionTest.cs
...
- Do not change any assertions, test data, or test names. In particular, do not add assertions about `Card.RequiredTargetCount` or the new multi-target selection flow here — those are introduced in later Phase B/C tasks (MTC-*/POC-*) and this file's existing single-target `GetValidActions` assertions must keep passing unchanged.

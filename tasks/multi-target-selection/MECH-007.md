...
Title:
Migrate Target -> Targets in Actions\DrawCardFromDeckAction.cs (DrawTargetCardFromDeckAction + AddCardToDeckAction)
...
Files:
CardBattleEngine\Actions\DrawCardFromDeckAction.cs

Requirements:
This file contains two classes (`DrawTargetCardFromDeckAction` and `AddCardToDeckAction`) — apply the same substitution rules to `.Target` usages in both:

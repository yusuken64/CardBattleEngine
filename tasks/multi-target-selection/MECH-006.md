...
Title:
Migrate Target -> Targets in Actions\AttackAction.cs
...
Requirements:
Apply these substitution rules to every occurrence of `.Target` (singular) in this file. In particular, wherever `context.Target` is passed as an argument into `AttackBehavior.CanAttack(...)` or `AttackBehavior.GenerateDamageActions(...)` (or similarly named attack-behavior methods), replace that argument expression with `context.Targets?.FirstOrDefault()`.
...
- Do not touch any other file, including `AttackBehaviors\HeroAttackBehavior.cs` and `AttackBehaviors\MinionAttackBehavior.cs` (those are separate tasks: MECH-011, MECH-012).

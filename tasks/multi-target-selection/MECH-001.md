...
Title:
Migrate Target -> Targets in Actions\FreezeAction.cs
...
Files:
CardBattleEngine\Actions\FreezeAction.cs
...
Acceptance Criteria:
- No bare `.Target` (singular) reference remains anywhere in this file (searching for `.Target` should only match `.Targets`, `.TargetOwner`-style unrelated identifiers, etc. — not the old singular field).

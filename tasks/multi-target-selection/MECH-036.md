...
Title:
Migrate Target -> Targets in CardBattleEngine.Test\TargetTest.cs
...
Files:
CardBattleEngine.Test\TargetTest.cs
...
- Do not add a multi-target test here — that belongs to a separate later task (POC-002, in a new file `MultiTargetTest.cs`), specifically to avoid merge risk with this mechanical rename task.
